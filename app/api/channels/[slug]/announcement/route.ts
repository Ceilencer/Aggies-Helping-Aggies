...
async function resolveChannel(supabase: Awaited<ReturnType<typeof createClient>>, slug: string) {
  ...
  if (error || !channel) {
    const message = slug === 'home'
      ...
      : 'Channel not found'
    return { channel: null, errorResponse: NextResponse.json({ error: message }, { status: 404 }) }
  }

  return { channel, errorResponse: null }
}
...
export async function GET(
  ...
  try {
    ...
    const { channel, errorResponse } = await resolveChannel(supabase, slug)
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await deleteChannelAnnouncement(supabase, channel!.id)
    if (error) {
    ...
    console.error('Error in DELETE /api/channels/[slug]/announcement:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
