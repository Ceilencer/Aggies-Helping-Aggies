...
export const getCachedUserProfile = cache(async function getCachedUserProfile(userId: string, supabase: SupabaseClient) {
  const { data, error } = await supabase
    .from('profiles')
    .select('id, email, full_name, avatar_url, role, flair, is_verified, is_alumni, rules_acknowledged_at, account_status, approved_by, approved_at, created_at, updated_at, graduation_year, major, last_login')
    .eq('id', userId)
    .maybeSingle()
